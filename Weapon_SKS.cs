datablock AudioProfile(GCats_SKSFireSound)
{
   filename    = "./Sounds/SKS.wav";
   description = MediumClose3D;
   preload = true;
};

AddDamageType("GCats_SKS",'<bitmap:Add-ons/Weapon_AEBase_GCatsSpecialWeapons/Icons/CI_SKS> %1','%2 <bitmap:Add-ons/Weapon_AEBase_GCatsSpecialWeapons/Icons/CI_SKS> %1',0.2,1);

//////////
// item //
//////////
datablock ItemData(GCats_SKSItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./SKS/SKS.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "SW: SKS";
	iconName = "./Icons/icon_SKS";
	doColorShift = true;
	colorShiftColor = "1 1 1 1";

	 // Dynamic properties defined by the scripts
	image = GCats_SKSImage;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(GCats_SKSImage)
{
   // Basic Item properties
	shapeFile = "./SKS/SKS.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = GCats_SKSSafetyImage;
    scopingImage = GCats_SKSIronsightImage;
	doColorShift = true;
	colorShiftColor = GCats_SKSItem.colorShiftColor;//"0.400 0.196 0 1.000";
    ...
    directDamageType = $DamageType::GCats_SKS;
    directHeadDamageType = $DamageType::GCats_SKS;
   ...
   // Initial start up state
	...
	stateName[6]				= "Reload";
	stateTimeoutValue[6]			= 1.2; //-0.15 for magazine weps since reloadend is now 0.25 instead of 0.1
...
function GCats_SKSImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, GCats_SKSFireSound);
...
function GCats_SKSImage::onDryFire(%this, %obj, %slot)
{
	serverPlay3D(GCats_EmptySound, %obj.getHackPosition());
}

function GCats_SKSImage::onReloadEnd(%this,%obj,%slot)
{
...
function GCats_SKSImage::onReloadStart(%this,%obj,%slot)
{
   %obj.aeplayThread(2, shiftRight);
}

function GCats_SKSImage::onReady(%this,%obj,%slot)
{
...
function GCats_SKSImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function GCats_SKSImage::onUnMount(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(GCats_SKSIronsightImage : GCats_SKSImage)
{
	recoilHeight = 0.1625;

	scopingImage = GCats_SKSImage;
	sourceImage = GCats_SKSImage;
	
	isScopedImage = true;
	
	offset = "0 0 0";
	eyeOffset = "0 0.15 -0.28";
...
function GCats_SKSIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function GCats_SKSIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function GCats_SKSIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, GCats_SKSFireSound);
...
function GCats_SKSIronsightImage::onDryFire(%this, %obj, %slot)
{
	serverPlay3D(GCats_EmptySound, %obj.getHackPosition());
}
...
function GCats_SKSIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function GCats_SKSIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
