datablock AudioProfile(GCats_XCRFireSound)
{
   filename    = "./Sounds/xcr.wav";
   description = MediumClose3D;
   preload = true;
};

AddDamageType("GCats_XCR",'<bitmap:Add-ons/Weapon_AEBase_GCatsSpecialWeapons/Icons/CI_xcr> %1','%2 <bitmap:Add-ons/Weapon_AEBase_GCatsSpecialWeapons/Icons/CI_xcr> %1',0.2,1);

//////////
// item //
//////////
datablock ItemData(GCats_XCRItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./XCR/xcr.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "SW: XCR";
	iconName = "./Icons/icon_xcr";
	doColorShift = true;
	colorShiftColor = "1 1 1 1";

	 // Dynamic properties defined by the scripts
	image = GCats_XCRImage;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(GCats_XCRImage)
{
   // Basic Item properties
	shapeFile = "./XCR/xcr.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = GCats_XCRSafetyImage;
    scopingImage = GCats_XCRIronsightImage;
	doColorShift = true;
	colorShiftColor = GCats_XCRItem.colorShiftColor;//"0.400 0.196 0 1.000";
    ...
    directDamageType = $DamageType::GCats_XCR;
    directHeadDamageType = $DamageType::GCats_XCR;
...
function GCats_XCRImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
    %obj.playAudio(0, GCats_XCRFireSound);
...
function GCats_XCRImage::onReloadEnd(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
	Parent::AEMagReloadAll(%this, %obj, %slot);
}

function GCats_XCRImage::onDryFire(%this, %obj, %slot)
{
...
function GCats_XCRImage::onReloadStart(%this,%obj,%slot)
{
   %obj.aeplayThread(2, shiftRight);
}

function GCats_XCRImage::onReady(%this,%obj,%slot)
{
...
function GCats_XCRImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function GCats_XCRImage::onUnMount(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(GCats_XCRIronsightImage : GCats_XCRImage)
{
	recoilHeight = 0.0625;

	scopingImage = GCats_XCRImage;
	sourceImage = GCats_XCRImage;
...
function GCats_XCRIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function GCats_XCRIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function GCats_XCRIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
    %obj.playAudio(0, GCats_XCRFireSound);
...
function GCats_XCRIronsightImage::onDryFire(%this, %obj, %slot)
{
	serverPlay3D(GCats_EmptySound, %obj.getHackPosition());
}
...
function GCats_XCRIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function GCats_XCRIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
