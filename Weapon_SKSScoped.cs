AddDamageType("GCats_SKSS",'<bitmap:Add-ons/Weapon_AEBase_GCatsSpecialWeapons/Icons/CI_SKSScoped> %1','%2 <bitmap:Add-ons/Weapon_AEBase_GCatsSpecialWeapons/Icons/CI_SKSScoped> %1',0.2,1);

//////////
// item //
//////////
datablock ItemData(GCats_SKSSItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./SKS/SKSS.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "SW: SKS Scoped";
	iconName = "./Icons/icon_SKSS";
	doColorShift = true;
	colorShiftColor = "1 1 1 1";

	 // Dynamic properties defined by the scripts
	image = GCats_SKSSImage;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(GCats_SKSSImage)
{
   // Basic Item properties
	shapeFile = "./SKS/SKSS.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = false;
	safetyImage = GCats_SKSSSafetyImage;
    scopingImage = GCats_SKSSIronsightImage;
	doColorShift = true;
	colorShiftColor = GCats_SKSSItem.colorShiftColor;//"0.400 0.196 0 1.000";
    ...
    directDamageType = $DamageType::GCats_SKSS;
    directHeadDamageType = $DamageType::GCats_SKSS;
...
function GCats_SKSSImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
...
function GCats_SKSSImage::onDryFire(%this, %obj, %slot)
{
	serverPlay3D(GCats_EmptySound, %obj.getHackPosition());
}

function GCats_SKSSImage::onReloadEnd(%this,%obj,%slot)
{
...
function GCats_SKSSImage::onReloadStart(%this,%obj,%slot)
{
   %obj.aeplayThread(2, shiftRight);
}

function GCats_SKSSImage::onReady(%this,%obj,%slot)
{
...
function GCats_SKSSImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function GCats_SKSSImage::onUnMount(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(GCats_SKSSIronsightImage : GCats_SKSSImage)
{
	recoilHeight = 0.1625;

	scopingImage = GCats_SKSSImage;
	sourceImage = GCats_SKSSImage;
...
function GCats_SKSSIronsightImage::onDone(%this,%obj,%slot)
{
	%obj.reloadTime[%this.sourceImage.getID()] = getSimTime();
	%obj.mountImage(%this.sourceImage, 0);
}

function GCats_SKSSIronsightImage::onReady(%this,%obj,%slot)
{
	%obj.baadDisplayAmmo(%this);
}

function GCats_SKSSIronsightImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(0); 
  %obj.playAudio(0, GCats_SKSFireSound);
...
function GCats_SKSSIronsightImage::onDryFire(%this, %obj, %slot)
{
	serverPlay3D(GCats_EmptySound, %obj.getHackPosition());
}
...
function GCats_SKSSIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function GCats_SKSSIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
